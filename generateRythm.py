...
def generate_leaf_midi(
    ...
    pad_notes     = build_note_list(root_note, scale_intervals, octaves=2)
    ...
    for bar in range(total_bars):
        ...
        for duration in drum_durations:
            ...
            drum_vel = int(random.gauss(90, 8))
            drum_vel = max(1, min(127, drum_vel))
            midi.addNote(TR_DRUMS, CH_DRUMS, note,
                         time=beat_time, duration=0.25, volume=drum_vel)
            beat_time += duration
            drum_hit_index += 1
 
    # ── 7b. Helper to write event list to a track ───────────────────────────
    def write_events(track, channel, events, start_beat=0.0, vel=90, variance=10):
        t = start_beat
        for pitch, duration in events:
            if pitch < 0 or pitch > 127:
                t += duration
                continue
            v = int(random.gauss(vel, variance))
            v = max(1, min(127, v))
    ...
    def write_pad_events(track, channel, events, start_beat=0.0,
                         vel=50, chord_threshold=0.5, variance=8):
        t = start_beat
        for pitch, duration in events:
            if pitch < 0 or pitch > 127:
                t += duration
    ...
    write_pad_events(TR_PAD, CH_PAD, pad_events, vel=80, chord_threshold=0.5)
